...
export default function Home() {
  ...
  useEffect(() => {
    userDetails.name.trim() !== "" && setErrorMessage({ ...errorMessage, name: '' });
    userDetails.email.trim() !== "" && setErrorMessage({ ...errorMessage, email: '' });
    userDetails.phone.trim() !== '' && setErrorMessage({ ...errorMessage, phone: '' });
  ...
  useEffect(() => {
    ...
    if (storedValues) {
      setUserDetails(JSON.parse(storedValues));
    }
  }, [])

  useEffect(() => {
    if (userDetails.email !== '' && userDetails.name !== '' && userDetails.phone !== '')
      localStorage.setItem('formvalues', JSON.stringify(userDetails))
  }, [userDetails])
    ...
    <div className="flex flex-col md:mt-0 bg-yellow z-20 absolute md:relative top-40 md:top-0">
      <div className="flex flex-col px-6 w-11/12 rounded-xl md:rounded-none md:px-8 mx-auto pt-4 pb-4 md:w-full bg-white">
        ...
        <form className="grid gap-4" onSubmit={handleSubmit}>
          <div className="grid">
            <label htmlFor="Name" className="text-[#02295A] w-[17.563rem] flex justify-between md:w-[26.5rem] text-sm font-medium tracking-tight">Name
              {errorMessage && <p className="text-red-500">{errorMessage.name}</p>}
            </label>
            <input value={userDetails.name} onChange={(e) => setUserDetails({ ...userDetails, name: e.target.value })} className="mt-[.5rem] border-[0.063rem] font-bold text-sm border-[#d7d6e6] text-[#02295a] w-[17.563rem] md:w-[26.5rem] h-[2.625rem] py-2 px-3 rounded-lg placeholder:font-medium placeholder:text-[#9699ab] focus-visible:outline-none focus-visible:border-[#473dff]" type="text" placeholder="e.g. Stephen King" required />
          </div>
          <div className="grid">
            ...
            <input value={userDetails.email} onChange={(e) => setUserDetails({ ...userDetails, email: e.target.value })} className="mt-[.5rem] border-[0.063rem] font-bold border-[#d7d6e6] text-[#02295a] w-[17.563rem] md:w-[26.5rem] h-[2.625rem] py-2 px-3 rounded-lg placeholder:font-medium placeholder:text-[#9699ab] focus-visible:outline-none focus-visible:border-[#473dff]" type="email" placeholder="e.g. stephenking@lorem.com" required />
          </div>
          <div className="grid">
            ...
            <input value={userDetails.phone} onChange={(e) => setUserDetails({ ...userDetails, phone: e.target.value })} className="mt-[.5rem] border-[0.063rem] font-bold border-[#554cd3] text-[#02295a] w-[17.563rem] md:w-[26.5rem] h-[2.625rem] py-2 px-3 rounded-lg placeholder:font-medium placeholder:text-[#9699ab] focus-visible:outline-none focus-visible:border-[#473dff]" type="text" placeholder="e.g. +1 234 567 890" required />
          </div>

...
const Summary = () => {
    ...
    function totalPrice() { 
        if (selectedAddonPrice.online && selectedAddonPrice.largerStorage && selectedAddonPrice.customizable) {
            const totalAddonPrice = selectedAddonPrice.online + selectedAddonPrice.customizable + selectedAddonPrice.largerStorage
            return totalAddonPrice + newPrice
        }
    }
    useEffect(() => {

        console.log({ selectedAddon }) //why does this return false here while true in add-ons page and localStorage
        displayedPrice()
    }, [])
        ...
        <div className="rounded-lg shadow-lg md:shadow-none -ml-2 z-20 flex-1 absolute md:relative top-40 md:top-0 p-1">
            <div className="flex flex-col px-6 w-11/12 rounded-xl md:rounded-none md:px-8  mx-auto pt-4 pb-4 md:w-full bg-red-white">
                ...
                <div className='p-5 flex justify-between items-center'>
                    ...
                    <p className='font-bold text-[#473DFF] text-lg'>+${totalPrice()}{tenure === "yearly" ? '/yr' : '/mo'}</p>
                </div>
